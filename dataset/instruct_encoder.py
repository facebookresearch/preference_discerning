...
def get_instruction_item(model):
    if 'flan-t5' in model:
        return "Item: "
    elif 'roberta' in model:
        return ""
    else:
        return f"Represent the items for retrieval: "

def get_instruction_preference(model):
    if 'flan-t5' in model:
        return "Instructions: "
    elif 'roberta' in model:
        return ""
...
class InstructEncoderModel(torch.nn.Module):
    def __init__(
            self,
        ...
        if is_inference:
            ...
            self.model.eval()
            if not ("device_map" in kwargs) and not (kwargs.get("load_in_4bit", False)) and not (
